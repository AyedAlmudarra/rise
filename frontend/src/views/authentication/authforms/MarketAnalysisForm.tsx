...
const CompetitorCard: React.FC<{
  ...
  // Watch values for threat level to set color
  ...
  return (
    <motion.div
      ...
      className="relative"
    >
      <div className="absolute inset-0 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-950/30 dark:to-gray-950/30 rounded-xl blur-sm opacity-60"></div>
      <motion.div
        whileHover={{ y: -4, boxShadow: "0 10px 20px -5px rgba(0,0,0,0.1)" }}
        transition={{ type: "spring", stiffness: 500, damping: 30 }}
      >
        <Card className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-gray-200 dark:border-gray-800 overflow-hidden hover:shadow-md transition-all duration-300 hover:border-blue-200 dark:hover:border-blue-800 rounded-xl">
          <CardContent className="p-5">
            ...
            <motion.button
              ...
              className="flex items-center mt-4 text-xs text-blue-600 dark:text-blue-400 font-medium py-2 px-3 rounded-md bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors"
...
// Interactive competitor comparison chart component
const CompetitorComparisonChart: React.FC = () => {
  const { watch } = useFormContext<StartupRegistrationData>();
  
  const competitor1Name = watch('competitor1Name');
  const competitor2Name = watch('competitor2Name');
  const competitor3Name = watch('competitor3Name');
  const competitor1Threat = watch('competitor1Threat');
  const competitor2Threat = watch('competitor2Threat');
  const competitor3Threat = watch('competitor3Threat');
  
  // Check if we have any data to show
  const hasData = competitor1Name || competitor2Name || competitor3Name;
  
  if (!hasData) {
    return (
      <div className="bg-gray-50 dark:bg-gray-900/30 rounded-lg border border-gray-200 dark:border-gray-800 p-4 text-center text-sm text-gray-500 dark:text-gray-400">
        Add competitor information to see a comparison
      </div>
    );
  }
  
  // Get threat badge color based on level
  const getThreatBadge = (level: string | undefined) => {
    switch (level) {
      case 'high': return <Badge className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border-red-200 dark:border-red-800">High Threat</Badge>;
      case 'medium': return <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300 border-amber-200 dark:border-amber-800">Medium Threat</Badge>;
      case 'low': return <Badge className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-200 dark:border-green-800">Low Threat</Badge>;
      default: return <Badge className="bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-300 border-gray-200 dark:border-gray-800">Unspecified</Badge>;
    }
  };
  
  const competitors = [
    { name: competitor1Name, threat: competitor1Threat, badge: getThreatBadge(competitor1Threat) },
    { name: competitor2Name, threat: competitor2Threat, badge: getThreatBadge(competitor2Threat) },
    { name: competitor3Name, threat: competitor3Threat, badge: getThreatBadge(competitor3Threat) }
  ].filter(comp => comp.name);
  
  return (
    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-lg border border-gray-200 dark:border-gray-800 p-4">
      ...
      <div className="space-y-3">
        {competitors.map((comp, index) => (
          <motion.div 
            ...
            className="p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-100 dark:border-gray-800 flex items-center justify-between"
          >
  ...
  const saveMarketAnalysis = async () => {
    ...
    try {
      ...
      if (isRegistrationFlow || window.location.pathname.includes('register')) {
        ...
        setSuccessMessage("Market analysis will be saved with your registration");
      ...
      if (!existingStartup) {
        // This is normal during initial registration
        setSuccessMessage("Market analysis will be saved with your registration");
        await triggerConfetti();
        setLoading(false);
        return;
      ...
      // Prepare competitor data in the format expected by the database
      ...
      // Update just the market analysis fields
    ...
    <div className="space-y-8">
      ...
      <AnimatePresence>
        {successMessage && (
          ...
          >
            <div className="absolute inset-0 bg-blue-400/20 dark:bg-blue-400/10 blur-xl rounded-lg"></div>
            <Alert className="bg-gradient-to-r from-blue-50/90 to-cyan-50/90 dark:from-blue-900/60 dark:to-cyan-900/60 border border-blue-200 dark:border-blue-800 text-blue-800 dark:text-blue-300 backdrop-blur-sm shadow-md animate-in fade-in-50 duration-300">
              <div className="flex items-center gap-3">
                <motion.div 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  transition={{ type: "spring", stiffness: 500, damping: 25, delay: 0.1 }}
                  className="bg-gradient-to-br from-blue-100 to-cyan-200 dark:from-blue-800/50 dark:to-cyan-700/50 p-2 rounded-full"
                >
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <TabsContent value="market-overview" className="mt-0 outline-none">
                  ...
                  >
                    <motion.div variants={itemVariants}>
                      <div className="relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-100 to-cyan-100 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-xl blur-sm opacity-60"></div>
                    ...
                    <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 rounded-xl blur-sm opacity-60"></div>
                        <Card className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-blue-100 dark:border-blue-900/30 overflow-hidden hover:shadow-md transition-all duration-300 rounded-xl">
                          <CardContent className="p-5">
                      ...
                      <div className="relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-cyan-50 to-teal-50 dark:from-cyan-950/20 dark:to-teal-950/20 rounded-xl blur-sm opacity-60"></div>
                        <Card className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-cyan-100 dark:border-cyan-900/30 overflow-hidden hover:shadow-md transition-all duration-300 rounded-xl">
                          <CardContent className="p-5">
                            <div className="flex items-center mb-4">
                    ...
                    <motion.div variants={itemVariants} className="space-y-6">
                      <div className="relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 rounded-xl blur-sm opacity-60"></div>
                        <Card className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-emerald-100 dark:border-emerald-900/30 overflow-hidden hover:shadow-md transition-all duration-300 rounded-xl">
                          <CardContent className="p-5">
                            <div className="flex items-center mb-4">
                ...
                <TabsContent value="competitor-analysis" className="mt-0 outline-none">
                  ...
                  >
                    <motion.div variants={itemVariants}>
                      <div className="relative">
                        ...
                        <div className="relative bg-white/40 dark:bg-gray-900/40 backdrop-blur-sm rounded-xl p-5 border border-indigo-200 dark:border-indigo-800 shadow-sm">
                          ...
                          {/* Toggle for comparison chart */}
                          <div className="mt-4 flex justify-center">
                            <motion.button
                              whileHover={{ scale: 1.03 }}
                              whileTap={{ scale: 0.97 }}
                              className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400 font-medium py-2 px-4 rounded-lg bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
                    ...
                    <motion.div 
                      ...
                      className="flex items-start p-4 bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-950/30 dark:to-yellow-950/30 rounded-lg border border-amber-200 dark:border-amber-800/50 shadow-sm mt-4"
                    >
              ...
              <div className="mt-8 flex justify-end">
                <motion.button 
                  ...
                  disabled={loading}
                  className="relative overflow-hidden bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white shadow-md px-5 py-2.5 rounded-md font-medium text-sm flex items-center"
                ...
                >
                  ...
                  <div className="relative flex items-center">
                    {loading ? (
                      <>
                        <div className="absolute inset-0 bg-blue-700/20 animate-pulse rounded-md"></div>
